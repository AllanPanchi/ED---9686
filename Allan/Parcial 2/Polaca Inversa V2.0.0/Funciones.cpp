...
double seno(double valor) {
    ...
    valor = fmod(valor, M_PI);

    for (int n = 0; n < terminos; n++) {
        double numerador = potenciaTrigonometrica(-1, n);
...
double potenciaTrigonometrica(double base, double exponente){
    if (exponente <= 0) {
        return 1.0;
    }
    else{
...
double potencia(double base, double exponente) {
    ...
    double logBase = calcularLogaritmoNatural(base);
...
double calcularLogaritmoNatural(double valor) {
    double epsilon = 1e-10;
...
double raizCuadrada(double valor){
    if (valor == 0.0) {
        return 0.0;
    }
...
double raizCubica(double valor){
    double epsilon = 1e-10;
