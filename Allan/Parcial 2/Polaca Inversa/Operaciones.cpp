...
public:
    ...
    bool funTrigonometrica(string c) {
        if (c == "sen" || c == "cos" || c == "tan" || c == "raiz" || c == "raizcubica") {
            return true;
        }
        return false;
    }
    ...
    double calcular(Pila<string> prefija) {
        ...
        try {
            ...
            while (caracter) {
                ...
                else if(caracter1 == "raizcubica"){
                    aux = p.getPrimero()->getValor();
                    p.Pop();

                    p.Push(to_string(Funciones::raizCubica(stod(aux))));//raiz cubica
                }
